...
// Whether a challenge should appear in the browse list for this person.
// Mirrors the row level security policy; the database is the real gate, this
// just avoids showing things the server would refuse anyway.
export function canSee(challenge, me, myGymId, invitedIds = [], memberIds = []) {
  if (!challenge) return false
  if (challenge.created_by === me) return true
  if (memberIds.includes(challenge.id)) return true
  if (invitedIds.includes(challenge.id)) return true
  if (challenge.visibility === 'public') return true
  if (challenge.visibility === 'gym') return !challenge.gym_id || challenge.gym_id === myGymId
  return false
}
